...
class PulseWaves(object):
    ...
    def __init__(self, pls_file):
        ...
        self.file_sig = "".join([b.decode(
            'utf-8') for b in struct.unpack("c"*16, pulsebinary.read(16))]).strip("\x00")
        ...
        # self.sys_id = "".join(struct.unpack(
        #     "c"*64, pulsebinary.read(64))).strip("\x00")
        self.sys_id = pulsebinary.read(64).decode('utf-8').strip("\x00")
        # self.software = "".join(struct.unpack(
        #     "c"*64, pulsebinary.read(64))).strip("\x00")
        ...
        for num_vlr in range(self.num_vlr):
            ...
            # TODO: add additional vlr types
                ...
                vlr.record = "".join([b.decode('utf-8') for b in struct.unpack(
                    "c"*vlr.record_length, pulsebinary.read(vlr.record_length))]).strip("\x00")
    ...
    def create_spatial_index(self, overwrite=False):
        ...
        print("Generating spatial index.....%s points...this may take a while...." %
              self.num_pulses)
        ...
        for pulse_record in self.cycle_pulses(0, self.num_pulses):
            ...
            if pulse_record.pulse_number in (np.linspace(0, 1, 11) * self.num_pulses).astype(int):
                print("%s percent complete." %
                      int(100*pulse_record.pulse_number/float(self.num_pulses)))
    ...
    def get_spatial_points(self, x, y, distance):
        ...
        intersected_pulses = list(spatial_index.intersection(
...
class PulseRecord(object):
    ...
    def __init__(self, pulsebinary, pulse_number, header):
        ...
        pulsebinary.seek(header.offset_to_pulses +
                         pulse_number * header.pulse_size)

        self.gps_timestamp = header.t_scale * \
            struct.unpack("q", pulsebinary.read(8))[0] + header.t_offset
        self.offset_to_waves = struct.unpack("q", pulsebinary.read(8))[0]
        self.x_anchor = header.x_scale * \
            struct.unpack("=l", pulsebinary.read(4))[0] + header.x_offset
        self.y_anchor = header.y_scale * \
            struct.unpack("=l", pulsebinary.read(4))[0] + header.y_offset
        self.z_anchor = header.z_scale * \
            struct.unpack("=l", pulsebinary.read(4))[0] + header.z_offset
        self.x_target = header.x_scale * \
            struct.unpack("=l", pulsebinary.read(4))[0] + header.x_offset
        self.y_target = header.y_scale * \
            struct.unpack("=l", pulsebinary.read(4))[0] + header.y_offset
        self.z_target = header.z_scale * \
            struct.unpack("=l", pulsebinary.read(4))[0] + header.z_offset
        ...
        self.pulse_descriptor = 200000 + \
            int("".join(str(x) for x in bits[:8][::-1]), 2)
...
class Waves(object):
    ...
    def __init__(self, header, pulse_record):
        ...
        self.file_sig = "".join(struct.unpack(
            "c"*16, wavebinary.read(16))).strip("\x00")
        ...
        for key in sample_records.keys():
            ...
            duration_anchor = struct.unpack(
                "=L", wavebinary.read(sample_record.bits_anchor/8))[0]
            num_samples = struct.unpack(
                "=h", wavebinary.read(sample_record.bits_samples/8))[0]

            samples = []
            for sample_num in range(num_samples):

                sample = struct.unpack("=h", wavebinary.read(
                    sample_record.bits_per_sample/8))[0]
                # calculate 3 dimensional sample coordinates
                x = pulse_record.x_anchor + \
                    (duration_anchor + sample_num) * pulse_record.dx
                y = pulse_record.y_anchor + \
                    (duration_anchor + sample_num) * pulse_record.dy
                z = pulse_record.z_anchor + \
                    (duration_anchor + sample_num) * pulse_record.dz
    ...
    def plot(self, save_path=None):
        ...
        ax_outgoing.set_xlabel(
            r"$\mathrm{\mathsf{Outgoing\/waveform}}$", fontsize=12)
        ax_outgoing.set_ylabel(r"$\mathrm{\mathsf{Z}}$", fontsize=12)
        ax_returning.plot(wave.segments[1][3], wave.segments[1][2], 'r')
        ax_returning.set_xlabel(
            r"$\mathrm{\mathsf{Returning\/waveform}}$", fontsize=12)
    ...
    def smooth(self, window, polyorder, deriv=0):
        ...
        for key, value in self.segments.iteritems():
            self.segments[key][3] = savgol_filter(
                self.segments[key][3], window, polyorder, deriv)
...
class VLR(object):
    ...
    def __init__(self, pulsebinary):
        self.user_id = pulsebinary.read(16).decode('utf-8').strip("\x00")
        ...
        self.desciption = pulsebinary.read(64).decode('utf-8').strip("\x00")
...
class Scanner(object):
    ...
    def __init__(self, pulsebinary):
        ...
        self.reserved = struct.unpack("I", pulsebinary.read(4))[0]
        self.instrument = "".join(struct.unpack(
            "c"*64, pulsebinary.read(64))).strip("\x00").strip("\x00")
        self.serial = "".join(struct.unpack(
            "c"*64, pulsebinary.read(64))).strip("\x00").strip("\x00")
        ...
        self.description = pulsebinary.read(64).decode('utf-8').strip("\x00")
